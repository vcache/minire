...
{
    FrustumPlanes precalcFrustumPlanes(ViewFrustum const & viewFrustum)
    {
        // frustum AABB
                     ...
                     FrustumPlanes const & frustumPlanes)
    {
        ...
        if (frustumPlanes._max.x < worldAabb.min().x || frustumPlanes._min.x > worldAabb.max().x) return false;
        if (frustumPlanes._max.y < worldAabb.min().y || frustumPlanes._min.y > worldAabb.max().y) return false;
        if (frustumPlanes._max.z < worldAabb.min().z || frustumPlanes._min.z > worldAabb.max().z) return false;

        // test World AABB against the 6 Frustum Planes (SAT Phase 2)
        for (size_t i = 0; i < frustumPlanes._planes.size(); ++i)
        {
            glm::vec3 const n = glm::vec3(frustumPlanes._planes[i]);
            float const d = frustumPlanes._planes[i].w;
